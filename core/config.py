...
class LayoutPreset(str, Enum):
    """Пресеты раскладки 9:16 (см. SPEC → Раскладки).

    ⚠️ Вебка есть НЕ У ВСЕХ: пресет E — для стримеров без камеры (и для моментов,
    где камеру не хочется показывать). Вебку можно погасить и глазом в редакторе,
    но отдельный пресет избавляет от ручной возни.
    """
    ...
    E = "E"  # Без вебки: только геймплей на весь экран
...
def composition_from_preset(preset: "LayoutPreset",
    ...
    elif preset == LayoutPreset.E:          # без вебки: только геймплей
        gp = Placement(0.0, 0.0, 1.0, 1.0)
        wc = Placement(0.0, 0.0, 0.3, 0.2, visible=False)
